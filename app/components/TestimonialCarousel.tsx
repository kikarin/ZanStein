  ...
  useEffect(() => {
    const checkAdminRole = async () => {
      if (user) {
        ...
        if (
          userDoc.exists() &&
          userDoc.data()?.role?.toLowerCase() === "admin"
        ) {
          setIsAdmin(true);
        }
      }
      ...
      setTestimonials(
        testimonials.map((t) =>
          t.id === testimonialId
            ? {
                ...t,
                adminReply: { text: replyContent, timestamp: Timestamp.now() },
              }
            : t
        )
      );
      ...
      >
        {Array.from({ length: totalColumns }).map((_, colIndex) => (
          <div key={colIndex} className="flex flex-col gap-6">
            {testimonials
              .slice(
                colIndex * itemsPerRow,
                colIndex * itemsPerRow + itemsPerRow
              )
              .map((t) => (
                <motion.div
                  key={t.id}
                  ...
                  <div className="p-6 relative">
                    ...
                    {t.adminReply && (
                      <div className="mt-6 flex items-start gap-4">
                        ...
                        {/* Kotak Balasan */}
                        <div className="admin-reply-box w-full">
                          <div className="flex items-center gap-2 mb-1">
                            <FaRobot className="admin-reply-icon shrink-0" />
                            <span className="admin-reply-text">Reply:</span>
                          </div>
